        ...
        for t=1:Nt-1
           ...
           % [Todo] x_ref at current time
            if Task.dt*t< t1
                if ~via_pt_lineariz
                    x_ref = p1;
                    A_lin = Model.Alin{1}(x_ref,u_ref,Model.param.syspar_vec);
                    B_lin = Model.Blin{1}(x_ref,u_ref,Model.param.syspar_vec);
                    [K_via, ~, ~] = lqr(A_lin, B_lin, Task.cost.Q_lqr, Task.cost.R_lqr, []);
                    via_pt_lineariz = 1;
                end
            theta_k = [u_ref + K_via*x_ref -K_via]';
            else
            x_ref = x_lin;
            theta_k = [u_ref + K*x_ref -K]';
            end
           % ==============================================================
